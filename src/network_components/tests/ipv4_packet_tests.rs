...
#[test]
fn ipv4_options() {
    let ipv4_data_in_u8: [u8; 61] = [74, 0, 0, 61, 177, 29, 64, 0, 64, 17, 128, 107, 192, 168, 1, 90, 142, 250, 184, 42, 131, 149, 1, 187, 0, 41, 100, 18, 82, 18, 246, 2, 24, 57, 214, 254, 202, 113, 65, 255, 85, 173, 50, 221, 178, 53, 134, 231, 184, 197, 223, 157, 159, 28, 221, 181, 199, 230, 164, 142, 134];
    let ipv4_packet = IPv4Packet::new(&ipv4_data_in_u8[..]);
    ...
    assert_eq!(ipv4_packet.ip_addr_src, IPv4Address::new(&[192, 168, 1, 90]));
    assert_eq!(ipv4_packet.ip_addr_dst, IPv4Address::new(&[142, 250, 184, 42]));
    assert_eq!(ipv4_packet.options, Vec::from(&ipv4_data_in_u8[20..40]));
    assert_eq!(ipv4_packet.payload, Vec::from(&ipv4_data_in_u8[40..]));
}
